...
/**
 * Функция дла передачи статистики чатов за данный период
 * Новых
 * Всего
 * Минимум
 * Максимум
...
module.exports = function get_period_data(socket, range) {
    ...
    /**
     * SELECT count(chat_id) FROM chats_stats WHERE registration_date >= moment() - range
     *    SELECT sum(count), min(count), max(count) FROM chats_stats WHERE id = (SELECT max(id) FROM chats_stats) - range
     */
